...
//livox_ros_driver::CustomMsgConstPtr
void lidarCallBackHorizon(const sensor_msgs::PointCloud2::ConstPtr &msg) {

  sensor_msgs::PointCloud2 msg2;
  // std::cout << "callback" << std::endl;

  if(Use_seg){
    // std::cout << "Use_seg" << std::endl;
  ...
  pcl::toROSMsg(*laserConerCloud, laserConerCloudMsg);
  pcl::toROSMsg(*laserSurfCloud, laserSurfCloudMsg);
  pcl::toROSMsg(*laserNonFeatureCloud, laserNonFeatureCloudMsg);

  laserCloudMsg.header = msg->header;
  // laserCloudMsg.header.stamp.fromNSec(timeSpan*1e9);

  // laserCloudMsg.header.stamp.fromNSec(msg->header.stamp.toSec());//?
  std::cout << msg->header.stamp.toSec() << std::endl;
  laserConerCloudMsg.header = msg->header;
  // laserConerCloudMsg.header.stamp.fromNSec(msg->header.stamp.toSec());
  laserSurfCloudMsg.header = msg->header;
  // laserSurfCloudMsg.header.stamp.fromNSec(msg->header.stamp.toSec());
  laserNonFeatureCloudMsg.header = msg->header;
  // laserNonFeatureCloudMsg.header.stamp.fromNSec(msg->header.stamp.toSec());
  // laserCloudMsg.header.stamp.fromNSec(msg->timebase+msg->points.back().offset_time);

  pubFullLaserCloud.publish(laserCloudMsg);
  pubSharpCloud.publish(laserConerCloudMsg);
  pubFlatCloud.publish(laserSurfCloudMsg);
...
int main(int argc, char** argv)
{
  ...
  if (Lidar_Type == 0)
  {
    customCloud = nodeHandler.subscribe<sensor_msgs::PointCloud2>(my_topic, 100, &lidarCallBackHorizon);
