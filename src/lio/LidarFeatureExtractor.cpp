...
void LidarFeatureExtractor::FeatureExtract_with_segment(const sensor_msgs::PointCloud2::ConstPtr &msg,
  ...
  // add this
  PointCloudEX::Ptr pcloud(new PointCloudEX);
  pcl::fromROSMsg(*msg, *pcloud);
 
  int dnum = pcloud->points.size(); //change here

  int *idtrans = (int*)calloc(dnum, sizeof(int));
  float *data=(float*)calloc(dnum*4,sizeof(float));
  int point_num = 0;

  double timeSpan = pcloud->points.back().t_sec+ pcloud->points.back().t_usec / 1000000.0 - pcloud->points.front().t_sec- pcloud->points.front().t_usec / 1000000.0;
  // double timeSpan = ros::Time().fromNSec(msg->points.back().offset_time).toSec();//change here
  PointType point;
  for(const auto& p : pcloud->points){
    // int line_num = (int)p.line;//change here

    int line_num = (int)p.channel;
    ...
    // point.normal_x = ros::Time().fromNSec(p.offset_time).toSec() /timeSpan; //change here
    point.normal_x = (pcloud->points.back().t_sec+ pcloud->points.back().t_usec / 1000000.0 - p.t_sec - p.t_usec / 1000000.0)/timeSpan;//?
